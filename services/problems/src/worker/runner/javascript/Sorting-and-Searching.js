...
function normalizeOutput(output) {
  // For sorting/searching problems, return output directly or null if undefined
...
(async () => {
  try {
    ...
    // Parse input JSON: could be an array of arguments or a single argument
    const input = JSON.parse(rawInput);

    let fnArgs;
    if (Array.isArray(input)) {
      fnArgs = input; // Spread input array as multiple arguments
    } else {
      fnArgs = [input]; // Single argument
    }
